...
standardizeRawDataConcRange <- function(sens.info, sens.raw) {
  unq.drugs <- unique(sens.info$treatmentid)

  conc.m <- data.table(melt(sens.raw[, , 1], as.is = TRUE))
  conc.m[, treatmentid := sens.info$treatmentid[match(Var1, rownames(sens.info))]]
  conc.ranges <- conc.m[, .(l = min(value, na.rm = T), r = max(value, na.rm = T)), c("treatmentid", "Var1")]
  conc.ranges[, Var1 := NULL]
  conc.ranges <- conc.ranges[, unique(.SD), treatmentid]
  # conc.ranges[,N := .N, treatmentid]
  ...
  per.drug.range.indicies <- sapply(conc.ranges.disj[, .N, treatmentid][, N], returnConsInts)

  names(per.drug.range.indicies) <- conc.ranges.disj[, unique(treatmentid)] ## checked this: conc.ranges.disj[,.N,treatmentid][,treatmentid] == conc.ranges.disj[,unique(treatmentid)]
  ...
  per.drug.range.indicies <- sapply(names(per.drug.range.indicies), function(drug) {
    lr.tbl <- conc.ranges.disj[treatmentid == drug]
  ...
  per.drug.range.indicies.2 <- sapply(names(per.drug.range.indicies), function(drug) {
    ...
    colnames(res) <- c("l", "r")
    res <- data.frame(res)
    res <- cbind(treatmentid = drug, res)
  }, simplify = FALSE)
  ...
  setkey(conc.m, treatmentid, Var1, value)
  setkey(conc.ranges, treatmentid, l, r)
  ...
  chosen.drug.ranges <- lapply(unq.drugs, function(drug) {
    num.points.in.range <- apply(per.drug.range.indicies.dt[treatmentid == drug, .(l, r)], 1, function(rng) {
      conc.m[treatmentid == drug][conc.ranges[treatmentid == drug][l <= rng["l"]][r >= rng["r"], Var1], on = "Var1"][value >= rng["l"]][value <= rng["r"], .N]
      # conc.m[treatmentid==drug][, Var1]
    })
    max.ranges <- per.drug.range.indicies.dt[treatmentid == drug][which(num.points.in.range == max(num.points.in.range))]
    max.ranges[which.max(log10(r) - log10(l)), ]
  })

  names(chosen.drug.ranges) <- sapply(chosen.drug.ranges, `[[`, "treatmentid")
  removed.experiments <- unlist(lapply(unq.drugs, function(drug) {
    rng <- unlist(chosen.drug.ranges[[drug]][, .(l, r)])
    exp.out.range <- conc.ranges[treatmentid == drug][l > rng["l"] | r < rng["r"], Var1]
  ...
  for (drug in unq.drugs) {
    rng <- unlist(chosen.drug.ranges[[drug]][, .(l, r)])
    myx <- conc.ranges.kept[treatmentid == drug, Var1]
    ...
    sens.info[sens.info$treatmentid == drug, "chosen.min.range"] <- rng["l"]
    sens.info[sens.info$treatmentid == drug, "chosen.max.range"] <- rng["r"]
  }
