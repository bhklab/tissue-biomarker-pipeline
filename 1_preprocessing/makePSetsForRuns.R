...
preprocessing_dir <- path("1_preprocessing")
source(path(preprocessing_dir, "filteringFunctions.R"))
...
inputDir <- path("Data", "unfilteredPSets")
filteredDir <- dir_create("Data", "TBPInputs", "filteredPSets")
...
CTRPv2.filtered.profiles.list <- PharmacoGx:::.calculateFromRaw(
  raw.sensitivity = CTRPv2.filtered.sens$sens.raw,
...
GRAY.filtered.profiles.list <- PharmacoGx:::.calculateFromRaw(
  raw.sensitivity = GRAY.filtered.sens$sens.raw,
  nthread = 20, cap = 100, family = "normal"
)
...
GDSC1.filtered.profiles.list <- PharmacoGx:::.calculateFromRaw(
  raw.sensitivity = GDSC1.filtered.sens$sens.raw,
...
GDSC2.filtered.profiles.list <- PharmacoGx:::.calculateFromRaw(
  raw.sensitivity = GDSC2.filtered.sens$sens.raw,
  nthread = 20, cap = 100, family = "normal"
)
...
gCSI <- readRDS(file.path(inputDir, "gCSI.rds"))

gCSI.filtered.sens <- standardizeRawDataConcRange(gCSI@sensitivity$info, gCSI@sensitivity$raw)

gCSI.filtered.profiles.list <- PharmacoGx:::.calculateFromRaw(
  raw.sensitivity = gCSI.filtered.sens$sens.raw,
  nthread = 12, cap = 100, family = "normal"
)
...
CCLE.filtered.profiles.list <- PharmacoGx:::.calculateFromRaw(
  raw.sensitivity = CCLE.filtered.sens$sens.raw,
  nthread = 12, cap = 100, family = "normal"
)
...
UHNBreast.filtered.profiles.list <- PharmacoGx:::.calculateFromRaw(
  raw.sensitivity = UHNBreast.filtered.sens$sens.raw,
  nthread = 12, cap = 100, family = "normal"
)
